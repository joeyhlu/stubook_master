...
class CalcPage2 extends StatefulWidget {
  const CalcPage2({super.key});

  @override
  State<CalcPage2> createState() => _CalcPageState();
}

class _CalcPageState extends State<CalcPage2> {
  final _myBox = Hive.box('mybox');
  MarkDataBase2 db = MarkDataBase2();
  CourseDataBase dbCourse = CourseDataBase();
  GradesDataBase dbGrade = GradesDataBase();
  @override
  void initState() {
    if (_myBox.get("MARKCALC2") == null) {
      db.initialData();
    } else {
      db.loadData();
    }
    ...
    return Container(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          title: Text(
            dbCourse.courses.length < 5 ? dbCourse.courses[1] : 'Course 2',
